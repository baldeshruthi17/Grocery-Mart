...
// Predefined available riders/delivery boys for authorization testing
  ...
  if (urlString.includes("/api/")) {
    ...
    try {
      ...
      if (urlString.endsWith("/api/delivery/login") && init?.method === "POST") {
        ...
        const rider = DELIVERY_BOYS.find(
          r => r.email.toLowerCase() === body.email?.toLowerCase() && r.password === body.password
        );

        if (rider) {
          console.log("Rider Authenticated successfully!", rider);
          console.groupEnd();
          ...
          console.groupEnd();
          return createJsonResponse({ success: false, error: "Invalid email or rider password" }, 401, "Unauthorized");
        }
      }
