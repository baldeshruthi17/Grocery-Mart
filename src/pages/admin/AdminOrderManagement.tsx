              ...
              <tbody className="divide-y divide-slate-50 font-medium">
                {filteredOrders.map((o) => (
                  <tr key={o.id} className="hover:bg-slate-50/10 transition">
                    ...
                    {/* Paid Total & Status Column */}
                    <td className="p-4">
                      <p className="font-extrabold text-slate-950 font-display">₹{o.total}</p>
                      <div className="flex flex-col gap-1 mt-1">
                        <span className={`inline-block px-1.5 py-0.5 rounded text-[8.5px] font-extrabold text-center uppercase tracking-wider w-max border ${
                          o.payment_status === 'PAID'
                            ? 'bg-emerald-50 text-emerald-800 border-emerald-250'
                            : 'bg-amber-50 text-amber-800 border-amber-255 border-amber-200'
                        }`}>
          ...
          <div className="relative w-full max-w-lg bg-white rounded-3xl shadow-2xl p-6 md:p-8 overflow-y-auto max-h-[85vh] animate-in zoom-in-95 duration-200 text-left space-y-6">
            ...
            <div className="p-4 bg-slate-50 border border-slate-200/60 rounded-2xl text-xs space-y-3">
              <div className="flex justify-between items-center text-[10px] uppercase font-bold text-slate-400 tracking-wider">
                <span>Payment Mode: {selectedOrder.paymentMethod || 'COD'}</span>
              ...
              {selectedOrder.transaction_id && (
                <p className="text-[10px] font-mono font-extrabold text-slate-500 border-b pb-1">
                  TXN ID: <span className="text-slate-800 select-all">{selectedOrder.transaction_id}</span>
                </p>
              )}
              <div className="flex justify-between items-center whitespace-nowrap">
                ...
                <div className="text-right">
                  ...
                  <p className="text-lg font-black font-display text-brand-600">₹{selectedOrder.total}</p>
                </div>
              </div>
            </div>
            ...
            <div className="space-y-2 pt-2 border-t">
              ...
              >
                ...
                {DELIVERY_BOYS.map((boy) => {
                  const isInactive = (boy as any).status === 'inactive';
                  return (
                    <option key={boy.id} value={boy.id} disabled={isInactive}>
                      {boy.avatar} {boy.name} {isInactive ? ' 🔴 (INACTIVE)' : ''} ({boy.vehicle})
                    </option>
                  );
                })}
              </select>
