...
export const AdminHub: React.FC = () => {
  ...
  const [activeTab, setActiveTab] = useState<'dashboard' | 'products' | 'orders' | 'inventory' | 'riders' | 'add-rider'>('dashboard');
  const [isAddProductModalOpen, setIsAddProductModalOpen] = useState(false);

  // Sync activeTab state from both hash paths (e.g. #/admin/riders) and query parameters
  useEffect(() => {
    const hash = window.location.hash;
    const cleanHash = hash.split('?')[0];

    if (cleanHash === '#/admin/add-rider') {
      setActiveTab('add-rider');
    } else if (cleanHash === '#/admin/riders') {
      setActiveTab('riders');
    } else {
  ...
  return (
    <div className="flex flex-col lg:flex-row gap-8 pb-12 select-none">
      ...
      <aside className="w-full lg:w-64 shrink-0 self-start">
        <div className="bg-white border rounded-3xl p-5 space-y-6 shadow-xs text-left">
          ...
          <nav className="space-y-1 block">
            ...
            {/* Tab 5: Rider Management systems */}
            <button
              onClick={() => {
                setActiveTab('riders');
              ...
              className={`w-full text-left px-3.5 py-2.5 rounded-xl text-xs font-bold flex items-center gap-2.5 transition cursor-pointer ${
                activeTab === 'riders' || activeTab === 'add-rider'
                  ? 'bg-slate-900 text-white shadow-md'
                  : 'text-slate-650 hover:bg-slate-50'
              }`}
            >
      ...
      <main className="flex-1 min-w-0">
        ...
        {activeTab === 'riders' && (
          <AdminRiderManagement initialMode="list" />
        )}

        {activeTab === 'add-rider' && (
          <AdminRiderManagement initialMode="add" />
        )}
